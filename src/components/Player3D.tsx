...
export function Player3D({ config, isMoving, isServing = false, facing = 'right' }: Player3DProps) {
  ...
  return (
    <>
      ...
      `}</style>
      <div className={`relative flex flex-col items-center transition-all duration-200 ${flipClass} ${serveClass} ${isMoving && !isServing ? 'animate-[walkBounce_0.3s_ease-in-out_infinite]' : ''}`}>
        {/* Hat */}
        {config.hat === 'ChefHat' && (
